...
const BoardFormComponent = memo((props) => {
  let { data, type, show, onClose, createPost, updatePost, deletePost, editable = false, getPosts } = props;
  let formData = { type };
  ...
  const BoardFormView = () => {
    ...
    const handleCreate = async () => [await createPost(formData), await getPosts(type)];
    const handleUpdate = async () => [await updatePost({ ...formData, postId: data.postId }), await getPosts(type)];
    const handleDelete = async () => [await deletePost({ ...formData, postId: data.postId }), await getPosts(type)];
          ...
          <Form noValidate>
            ...
            <Modal.Body style={{ backgroundColor: "white", borderRadius: "5px" }}>
              ...
              {hasData && (
                <Row>
                  <Col className="col-7">
                    <Form.Text>{`Created: ${dateTime.boardDate(data.created)}`}</Form.Text>
